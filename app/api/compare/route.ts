...
export async function GET(req: NextRequest) {
  ...
  if (!asset && !assets.length && !vaults.length) {
    return NextResponse.json({ error: "provide ?asset=USDC, ?assets=USDC,WETH or ?vaults=0x..." }, { status: 400 });
  }

  const key = `compare:${chain}:${asset ?? ""}:${assets.sort().join("+")}:${vaults.sort().join(",")}`;
  try {
    const { data, cachedAt, fresh } = await cached(key, TTL, () => compare({ chain, asset, assets, vaults }), force);
